...
inline void CheckError(const cl_int status) {
  if (status != CL_SUCCESS) {
    throw std::runtime_error("Internal OpenCL error: ");
  }
}
...
class Platform {
 public:
  ...
  explicit Platform(const size_t platform_id) {
    ...
    if (platform_id >= num_platforms) { Error("invalid platform ID "); }
...
class Device {
 public:
  ...
  explicit Device(const Platform &platform, const size_t device_id) {
    ...
    if (device_id >= num_devices) { Error("invalid device ID "); }
  ...
  size_t VersionNumber() const
  {
    ...
    size_t version = (size_t) (100.0 * strtod(version_string.substr(0, next_whitespace).c_str(), NULL));
    return version;
  }
